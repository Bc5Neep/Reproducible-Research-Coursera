...
Date = activity$date Interval = activity\$interval SD \<- aggregate(Steps\~Date, data = activity, sum) SDI \<- aggregate(Steps\~Date + Interval, activity, sum)
...
![](Total Steps Taken Each Day_1.png)
...
abline(v = mean(SD$Steps), col ="red", lwd = 3) text( x = mean(SD$Steps),
      y = 5.5,
      labels = sprintf("Mean = %.2f", mean(SD$Steps)), col ="black", pos = 4) \# Median median(SD\$Steps)

![](Mean Total Steps Daily_1.png)

hist(SD$Steps, breaks = 53, main = "Total Steps Daily", xlab = "steps")
abline(v = median (SD$Steps), col = "black", lwd =5) text( x = median(SD$Steps),
      y = 5.5,
      labels = sprintf("Median = %.2f", median(SD$Steps)), col ="black", pos = 4)

![](Median Total Steps 1.png)
...
x1 <- Max1$Interval y1 \<- Max1\$Steps
...
![](Average Daily Step Activity.png)
...
options(max.print = 3000) data.frame(which(is.na(activity))) which(is.na(activity)) \# Identify and isolate NA
...
![With imputed values added from activity_1](Total Steps Taken Each Day Activity_1.png)
...
median(SD2\$steps) \# Generate new plots of mean and median total steps daily

hist(SD2$steps, breaks = 61, main = "Total Steps Daily", xlab = "steps")
abline(v = mean(SD2$steps), col = "red", lwd = 3) text( x = mean(SD2$steps),
      y = 5.5,
      labels = sprintf("Mean = %.2f", mean(SD2$steps)), col ="black", pos = 4)

hist(SD2$steps, breaks = 61, main = "Total Steps Daily", xlab = "steps")
abline(v = median (SD2$steps), col = "black", lwd =5) text( x = median(SD2$steps),
      y = 5.5,
      labels = sprintf("Median = %.2f", median(SD2$steps)), col ="black", pos = 4)

![With imputed values from activity_1](Mean Total Steps Daily Activity_1.png)

![With imputed values from activity_1](Median Total Steps Daily Activity_1.png)
...
x2 <- Max2$WD_Steps y2 \<- Max2\$WD_Interval

Max3 \<- SD_WE [which.max(SD_WE$WE_Steps),]
x3 <- Max3$WE_Steps y3 \<- Max3\$WE_Interval

# For weekday vs weekend time series
...
abline( v = 835, col ="red", lwd =2) text(x = 865, y = 190, labels = sprintf("Max Steps = %.2f at inteval 835", (x2)), adj = 0, cex =0.6)
...
![With imputed values from activity_1](Weekday v Weekend.png)
